...
class GroundTruthAlignedExtractor:
    ...
    def _analyze_document_structure(self, doc) -> Dict:
        ...
        analysis['document_type'] = self._determine_document_type(all_text_elements)
        analysis['has_numbered_sections'] = self._has_numbered_sections(all_text_elements)
        analysis['has_hierarchical_structure'] = self._has_hierarchical_structure(all_text_elements)
        
        # Find potential title candidates
        if all_text_elements:
            first_page_elements = [e for e in all_text_elements if e['page'] == 1]
            analysis['title_candidates'] = self._find_title_candidates(first_page_elements, analysis)
        
        # Find potential header candidates with more relaxed criteria
        analysis['potential_headers'] = self._find_potential_headers_relaxed(all_text_elements, analysis)
    ...
    def _determine_document_type(self, text_elements: List[Dict]) -> str:
        ...
        single_words = sum(1 for elem in text_elements if len(elem['text'].split()) <= 2)
    ...
    def _calculate_header_score_relaxed(self, elem: Dict, analysis: Dict) -> float:
        ...
        # Title-like patterns
            ...
            score += 0.3
        elif text.endswith(':') and len(text) <= 60:  # Section labels ending with colon
            score += 0.3
    ...
    def _extract_headers_from_structure(self, doc, analysis: Dict) -> List[Dict]:
        ...
        try:
            ...
            if doc_type == 'simple_document':
                ...
                potential_headers = [h for h in potential_headers if h['score'] >= 0.7]
            ...
            return filtered_headers[:40]  # Reasonable limit based on ground truth max
...
def process_all_test_files() -> Dict[str, Dict]:
            ...
            if result.get("outline"):
                ...
                for i, header in enumerate(result["outline"]):
                    ...
                    if i < 3:  # Show first 3 headers as samples
                        sample_headers.append(f"{level}: {header['text'][:40]}{'...' if len(header['text']) > 40 else ''}")
    ...
    # Save results
    output_file = "ground_truth_aligned_results.json"
    ...
    if os.path.exists("Ground_Truth.json"):
        ...
        for filename in sorted(results.keys()):
            if filename in ground_truth:
                ...
                print(f"{match_symbol} {filename}: GT={gt_headers}, Ours={our_headers}")
        
        print(f"\n📈 Overall Accuracy:")
        print(f"Exact matches: {exact_matches}/{len(results)} files")
        print(f"Total headers: GT={total_gt}, Ours={total_ours}")
        print(f"Header detection rate: {(total_ours/total_gt)*100:.1f}%" if total_gt > 0 else "N/A")
...
def main():
    ...
    try:
        ...
        if results:
            total_headers = sum(len(result.get("outline", [])) for result in results.values())
